  ...
  return (
    <>
      ...
      <section className="mx-10 md:mx-28 lg:mx-80 pt-16">
        ...
        {event.Summary && (
          <>
            {event.Summary.map((summary, index) => (
              <p key={index} className="leading-7 mb-10 text-justify">
                {summary}
              </p>
            ))}
          </>
        )}
        {event.wwstpComposeOf && (
          <>
            <h4 className="text-black font-bold text-xl mb-2">
              The Project is composed of
            </h4>
            <ol className="ml-6 list-decimal list-inside">
              {event.wwstpComposeOf.map((list, index) => (
                <li key={index}>{list}</li>
              ))}
            </ol>
          </>
        )}
        {event.limboComposeOf && (
          <>
            <h4 className="text-black font-bold text-xl mb-2">
              The Project is composed of
            </h4>
            <ul className="ml-6 space-y-8">
              {event.limboComposeOf.map((list, index) => (
                <li key={index}>{list}</li>
              ))}
            </ul>
          </>
        ...
        {event.wwstpOutput && (
          <>
            {Object.entries(event.wwstpOutput).map(([key, outputs], index) => (
              <div key={index} className="mb-8">
                <h4 className="font-bold">{key}</h4>
                <ul className="ml-20 list-disc list-inside">
                  {outputs.map((output, subIndex) => (
        ...
        {event.wfwwOutput && (
          <>
            {event.wfwwOutput.map((output, index) => (
              <p key={index} className="leading-7 mb-4 text-justify">
                {output}
              </p>
            ))}
          </>
